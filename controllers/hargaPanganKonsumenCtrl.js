var HargaPanganKonsumenSchema = require('../models/hargaPanganKonsumenModel');
var Data = HargaPanganKonsumenSchema.data;
var Histogram = HargaPanganKonsumenSchema.histogram;
...
exports.sortQueryByKey = function(req, res) {
  var query = Data.find({
    [req.params._key]: [req.params._value]
  });
  if (req.params._arg === 'desc') {
    query.sort({
      [req.params._key2]: 'desc'
    }).exec(function(err, resultJsons) {
      if (err) {
        res.status(500).send(err);
      } else {
...
exports.updateAll = function(req, res) {
    ...
    req.jsonData.wilayah = req.body.wilayah;
    req.jsonData.komoditi = req.body.komoditi;
    req.jsonData.harga = req.body.harga;
    req.jsonData.satuan = req.body.satuan;
